...
function [ rec_block, control_inf, control_inf_counter ] = ctrl_inf_block_extraction(block, control_inf, control_inf_counter, lsb_backup_size)
    ...
    if (control_inf_counter < lsb_backup_size)
        if dmax > 0
            if (dmax == 1 || dmax == 2)
                %disp(strcat('extracted from dmax=',num2str(dmax)));
                control_inf_counter = control_inf_counter + 1;
                b = dmax-1;
                control_inf(control_inf_counter) = num2str(b);
