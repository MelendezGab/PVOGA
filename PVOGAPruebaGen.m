...
max_fit = {};
for sim_num = 1:10    
clearvars -except sim_num max_fit
set_paths();
...
filePh = fopen(char(strcat(results_path,image_name,{' '},num2str(num_gen),{' '},'gen',{' '},num2str(sim_num),{' '}, regexprep(datestr(datetime('now')),':',','),'.txt')),'wt');
fprintf(filePh,'\n%s\n\n',strcat(image_path,image_name));
fprintf(filePh,'\n%s\n\n',char(strcat('Number of generations: ',{' '}, num2str(num_gen))));
...
fprintf(filePh,'%s\n',char(strcat(' --> Generation',{' '},num2str(current_gen-1),' -----------------------------------------------------------------------')));
current_population = pob{current_gen};
    for i = 1:4
        for j=1:4
...
fclose(filePh);
max_fit{end+1} = max(fitness(:));

if (sim_num==10)
    filePh = fopen(char(strcat(results_path,image_name,{' '},num2str(num_gen),{' '},'gen',{' '},'summary',{' '}, regexprep(datestr(datetime('now')),':',','),'.txt')),'wt');
    fprintf(filePh,'\n\n%s\n',strcat('Best PSNR 10 - simulations'));
    for i = 1:sim_num
       fprintf(filePh,'%s\n',num2str(cell2mat(max_fit(i))));
    end
    fprintf(filePh,'\n\n\n%s\n',char(strcat('PSNR min :',{' '},num2str(min(cell2mat(max_fit))))));
    fprintf(filePh,'%s\n',char(strcat('PSNR max :',{' '},num2str(max(cell2mat(max_fit))))));
    fprintf(filePh,'\n%s\n',char(strcat('mean     :',{' '},num2str(mean(cell2mat(max_fit))))));
    fprintf(filePh,'%s\n',char(strcat('std      :',{' '},num2str(std(cell2mat(max_fit))))));
    fclose(filePh);
end
end
